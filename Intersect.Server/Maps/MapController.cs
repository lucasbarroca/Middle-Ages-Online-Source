...
namespace Intersect.Server.Maps
{
    ...
    /// <remarks>
    /// A Map Controller is generally not responsible for sending packets to players, that is the job of the Controller's Map Instance.
    /// However, the exception to that case is whenever we want to perform some action for ALL players on a map, regardless of
    /// whatever Map Instance they are currently on. In those cases, that logic should live here, and generally means iterating
    /// over the map instances that belong to this <see cref="MapController"/>.
    /// <para>
    ...
    public partial class MapController : MapBase
    {
        ...
        public ICollection<Player> GetPlayersOnAllInstances()
        {
            ...
            foreach (var mapInstanceId in mInstances.Keys.ToList())
            {
                players.AddRange(mInstances[mapInstanceId].GetPlayers());
            }
            return players;
        }
        ...
        public List<Player> GetPlayersOnSharedInstance(Guid mapInstanceId, Entity except)
        {
            ...
            if (mInstances.TryGetValue(mapInstanceId, out var mapInstance))
            {
